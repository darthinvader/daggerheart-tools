...
// Mixed ancestry examples from the text
export const MIXED_ANCESTRY_EXAMPLES: MixedAncestry[] = [
  {
    name: 'Horned Toad',
    parentAncestries: ['Infernis', 'Ribbet'],
    primaryFeature: {
      name: 'Fearless',
      description:
        'When you roll with Fear, you can mark 2 Stress to change it into a roll with Hope instead.',
  ...
  {
    name: 'Flitterin',
    parentAncestries: ['Faerie', 'Simiah'],
    primaryFeature: {
      name: 'Luckbender',
      description:
        'Once per session, after you or a willing ally within Close range makes an action roll, you can spend 3 Hope to reroll the Duality Dice.',
  ...
  {
    name: 'Shroomshell',
    parentAncestries: ['Fungril', 'Galapa'],
    primaryFeature: {
      name: 'Fungril Network',
      description:
        'Make an Instinct Roll (12) to use your mycelial array to speak with others of your ancestry. On a success, you can communicate across any distance.',
...
export function getAncestryPrimaryFeatures(): AncestryFeature[] {
  return ANCESTRIES.map(ancestry => ancestry.primaryFeature);
}

export function getAncestrySecondaryFeatures(): AncestryFeature[] {
  return ANCESTRIES.map(ancestry => ancestry.secondaryFeature);
}
